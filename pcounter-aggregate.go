...
type DataStore interface {
	store(str string)
	timeStore(ts time.Time, str string)
}
...
var (
	...
	interval = flag.Int("interval", 0, "Interval of conversion. If minus, no emit, 0, realtime. Plus interval sec.")
	mu       sync.Mutex
	version  = "0.01"
	dataDir  string
	ds       DataStore
...
// open file with today info
func (fs *FileSystemDataStore) timeStore(day time.Time, str string) {
	const layout = "2006-01-02"
	day = day.In(jstZone)
	todayStr := day.Format(layout) + ".csv"
	//	log.Printf("DayFile %s", todayStr)
	if fs.todayStr != "" && fs.todayStr != todayStr {
		fs.storeFile.Close()
		fs.storeFile = nil
	}
...
func supplyIntervalAcounter(client *sxutil.SXServiceClient, ch chan *aggArea, inter int) {
	...
	for {
		...
		for id := range agg.Areas {
			area := &agg.Areas[id]
			if area.UpdateFlag {
				tmsp, _ := ptypes.Timestamp(area.Timestamp)
				ts := tmsp.Unix()
				if ts-lastTs > int64(inter) {
					lastTs = int64(ts/int64(inter)) * int64(inter)
					dataSend = true
					break
				}
			}
			...
			for id := range agg.Areas {
				...
				if area.UpdateFlag {
					...
					smo := sxutil.SupplyOpts{
						Name:  "ACounter:" + strconv.Itoa(inter),
						Cdata: &cont,
					}
		...
		mu.Unlock()
		//		time.Sleep(time.Millisecond * 100) // check for each second
	}
}
...
func supplyChannelAcounter(client *sxutil.SXServiceClient, ch chan *aggArea) {
	for {
		...
		line := fmt.Sprintf("%s,%s,%d,%d", ts, area.AreaName, area.AreaId, area.Count)
		fmt.Println(line)
		tm, _ := ptypes.Timestamp(area.Timestamp)
		ds.timeStore(tm, line)
...
func main() {
	...
	if *interval > 0 { // if interval > 0 then make a conversion.
		ac_client := sxutil.NewSXServiceClient(client, pbase.AREA_COUNTER_SVC, "{Client:PcountAgg}")
		go supplyIntervalAcounter(ac_client, areaChan, *interval)
	}
